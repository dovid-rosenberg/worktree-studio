  ...
  return {
    ...
    decorate({ path, repo }: Pick<Worktree, 'path' | 'repo'>, running: Map<string, RunningServer>) {
      ...
      return {
        ...
        ports: hit ? hit.ports : [],
        canStart: startable.includes(repo),
      };
    },
  };
}
  ...
  return [
    {
      ...
      worktrees: [
        ...
        {
          path: '/code/api/.worktrees/feat-a',
          name: 'feat-a',
          branch: 'feature/a',
          isMain: false,
          detached: false,
        ...
        {
          path: '/code/api/.worktrees/feat-b',
          name: 'feat-b',
          branch: 'feature/b',
          isMain: false,
          detached: false,
...
function build(over: BuildOver = {}) {
  ...
  return {
    state,
    cfg,
    setRepos: (r: StateRepo[]) => {
      repos = r;
    },
    setRunning: (m: Map<string, RunningServer>) => {
      running = m;
    },
  };
}
...
test('a worktree carries a trimmed view of its session; main checkouts never do', async () => {
  ...
  const sess = {
    ...session({ id: 's_1', state: 'waiting', activity: 'waiting for you', muxName: 'wts-a' }),
    secret: 'not exposed',
  };
  const { state } = build({
  ...
  assert.equal(
    ...
    null,
    'the main checkout is never session-decorated',
  );
});
...
test('a feature surfaces its driving session and its concurrency slot', async () => {
  ...
  const st = await state.buildState();
  const a = present(
    st.features.find((f) => f.name === 'feat-a'),
    'feature feat-a',
  );
  const b = present(
    st.features.find((f) => f.name === 'feat-b'),
    'feature feat-b',
  );
...
test('repos and running are read through getters, so a refresh is picked up', async () => {
  ...
  assert.equal(
    (await state.buildState()).runningTotal,
    1,
    'the replaced lsof map is seen, not a captured stale one',
  );
  setRepos([]);
...
test('conflictsFor lists only same-repo worktrees running elsewhere', async () => {
  ...
  assert.deepEqual(
    out.map((w) => w.path),
    ['/c'],
    'not itself, not a stopped sibling, not another repo',
  );
});
...
function onDisk(n: number): { root: string; targets: string[]; links: string[]; repos: StateRepo[] } {
  ...
  const links: string[] = [];
  for (let i = 0; i < n; i++) {
    const t = path.join(root, `target-${i}`);
    fs.mkdirSync(t);
    targets.push(t);
    const l = path.join(root, `wt-${i}`);
    fs.symlinkSync(t, l);
    links.push(l);
  }
  const worktrees = [
    { path: root, name: 'api', branch: 'main', isMain: true, detached: false, merged: false },
    ...links.map((l, i) => ({
      path: l,
      name: `feat-${i}`,
      branch: `b-${i}`,
...
test('every worktree path is resolved once, then served from cache on later builds', () => {
  const disk = onDisk(4);
  const sessions = disk.targets.map((t, i) =>
    session({ id: `s_${i}`, worktreePath: t, repos: [sessionRepo({ repo: 'api', worktreePath: t })] }),
  );
  const { state } = build({ repos: disk.repos, manager: realManager(sessions) });

  const cold = countRealpaths(() => state.topology());
  const warm = countRealpaths(() => {
    state.topology();
    state.topology();
    state.topology();
  });
...
test('sessionState shares the cache — a hook-driven build makes no syscalls', () => {
  ...
  assert.equal(
    countRealpaths(() => {
      for (let i = 0; i < 20; i++) state.sessionState();
    }),
    0,
...
test('prunePaths invalidates a worktree that was removed and recreated pointing elsewhere', () => {
  ...
  fs.mkdirSync(after);
  const link = path.join(root, 'feat');
  fs.symlinkSync(before, link);
  ...
  const manager = realManager([
    session({ id: 's_1', worktreePath: before, repos: [sessionRepo({ repo: 'api', worktreePath: before })] }),
  ]);
  let repos = withFeat;
  const deps = {
    cfg: { baseDirs: [root], web: { port: 0 }, groups: [] },
    manager,
    servers: fakeServers(),
    mux: { name: 'tmux' },
    repos: () => repos,
  ...
  manager.sessions.set(
    's_2',
    session({ id: 's_2', worktreePath: after, repos: [sessionRepo({ repo: 'api', worktreePath: after })] }),
  );
  repos = withFeat;

  assert.equal(sessionIdAt(pruned.topology(), 'feat'), 's_2', 'resolved fresh, not through the dead entry');
  assert.equal(
    sessionIdAt(never.topology(), 'feat'),
    null,
...
test('prunePaths keeps the entries the scan and the sessions still name', () => {
  ...
  state.prunePaths();
  assert.equal(
    countRealpaths(() => state.topology()),
    0,
    'a prune that removes nothing must not cost a rebuild',
  );
  fs.rmSync(disk.root, { recursive: true, force: true });
});
...
test('a corrupt sessions.json is kept aside instead of being replaced by an empty one', () => {
  ...
  assert.equal(
    fs.readFileSync(path.join(stateDir, kept), 'utf8'),
    original,
