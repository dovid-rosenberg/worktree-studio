...
/*
 * The daemon's dev address, and the ports this dev server binds. All overridable so a
 * second daemon can be run from a worktree without editing this file, and so two dev
 * servers don't fight over 5273:
 *
 *   WT_STUDIO_CONFIG=/tmp/x/config.json WT_STUDIO_STATE=/tmp/x/state node server/server.js
 *   WT_STUDIO_DAEMON=http://127.0.0.1:7803 WT_STUDIO_DEV_PORT=5281 \
 *     WT_STUDIO_STATE=/tmp/x/state npm run dev
 *
 * `WTS_DAEMON` is accepted as an alias for the daemon URL; the WT_STUDIO_ prefix matches
 * the env vars the server itself reads. With nothing set, every default is unchanged.
 */
const DAEMON = process.env.WT_STUDIO_DAEMON || process.env.WTS_DAEMON || 'http://127.0.0.1:7788';
const DEV_PORT = Number(process.env.WT_STUDIO_DEV_PORT || 5273);
const PREVIEW_PORT = Number(process.env.WT_STUDIO_PREVIEW_PORT || 5274);
