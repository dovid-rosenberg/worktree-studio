...
class Notify {
  ...
  prefs = $state({ waiting: true, sound: true, idle: false });
  ...
  onSessionFrame(frame) {
    ...
    this.waitingCount = sessions.filter((/** @type {any} */ s) => s.state === 'waiting').length;
  }

  /**
   * @param {any} s
   * @param {'waiting'|'idle'} kind
   */
  #fire(s, kind) {
 ...
 * @param {'waiting'|'idle'} kind
 */
function desktopNotification(s, kind) {
  if (!('Notification' in window)) return;
