...
function armed() {
  ...
  crash.install({
    log: (...a: unknown[]) =>
      logs.push(a.map((x) => (x instanceof Error ? `${errCode(x)} ${x.message}` : String(x))).join(' ')),
...
test('a dead client connection is the only survivable exception class', () => {
  for (const code of crash.CONNECTION_ERROR_CODES)
    assert.equal(crash.isConnectionError(err(code)), true, code);
  ...
  assert.equal(
    crash.isConnectionError(new Error('no code at all')),
    false,
    'a plain throw is a bug, not a lost socket',
  );
...
test('a client that vanished mid-write is survived, not fatal', () => {
  ...
  assert.ok(
    logs.every((l) => l.includes('continuing')),
    logs.join('\n'),
  );
});
...
test('guardListen turns a bind failure into an explained non-zero exit', () => {
  ...
  crash.guardListen(
    ...
    { log: (...a: unknown[]) => logs.push(String(a[0])), exit: (c: number) => exits.push(c) },
  );
...
test('listenErrorMessage names the remedy for each bind failure and ignores the rest', () => {
  assert.match(
    present(crash.listenErrorMessage(err('EADDRINUSE'), { host: '127.0.0.1', port: 4300 })),
    /already in use/,
  );
  assert.match(
    present(crash.listenErrorMessage(err('EACCES'), { host: '127.0.0.1', port: 80 })),
    /web\.port/,
  );
...
function through(err: unknown, { headersSent = false } = {}) {
  const logs: string[] = [];
  const sent: { status: number | null; body: unknown; nexted: unknown } = {
    status: null,
    body: null,
    nexted: undefined,
  };
...
test('a daemon that cannot bind its port exits non-zero instead of running on headless', {
  timeout: 60000,
}, async () => {
  ...
  let out = '';
  present(child.stdout).on('data', (d) => {
    out += d;
  });
  present(child.stderr).on('data', (d) => {
    out += d;
  });
