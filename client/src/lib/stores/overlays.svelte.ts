...
class Overlays {
  ...
  openIntake(): void { this.intake = true; }
  closeIntake(): void { this.intake = false; }
  openSettings(): void { this.settings = true; }
  closeSettings(): void { this.settings = false; }
  togglePalette(): void { this.palette = !this.palette; }
  closePalette(): void { this.palette = false; }

  /** True while anything blocking is on screen — global shortcuts stand down. */
  get any(): boolean { return this.palette || this.settings || this.intake || dialogs.queue.length > 0; }
