<script lang="ts">
  ...
  async function apply(a: any) {
    ...
    try {
      const res = await applyHunks(a.op, sessionId, {
        repo: sel.repo, file: a.file, hunks: a.hunks, expect: a.expect,
      });
  ...
  const blocks = $derived.by(() => {
      ...
      if (renameSummary) {
        ...
        if (!h) {
          ...
          if (f.parsed && f.parsed.hunks.length) {
            groups.push({ label: 'Working tree · loading staging state…', side: null, action: null, hunks: f.parsed.hunks });
          } else {
          ...
          error = h.error;
          if (f.parsed && f.parsed.hunks.length) {
            groups.push({ label: 'Working tree (staging unavailable)', side: null, action: null, hunks: f.parsed.hunks });
          }
        } else {
          const un = h.unstaged;
          const st = h.staged;
          if (un && un.hunks.length) {
            groups.push({ label: `Unstaged · ${un.hunks.length} hunk${un.hunks.length === 1 ? '' : 's'}${h.untracked ? ' · untracked' : ''}`, side: 'unstaged', action: 'stage', hunks: un.hunks });
          }
          if (st && st.hunks.length) {
            groups.push({ label: `Staged · ${st.hunks.length} hunk${st.hunks.length === 1 ? '' : 's'}`, side: 'staged', action: 'unstage', hunks: st.hunks });
          }
      ...
      return {
        ...
        rename: f.parsed && f.parsed.status === 'renamed' && f.parsed.oldPath && f.parsed.newPath
          ? `⟵ ${f.parsed.oldPath}` : null,
