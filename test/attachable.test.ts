...
const wt = (name: string, path: string, over: Record<string, unknown> = {}) =>
  ({ name, path, branch: `fix/${name}`, isMain: false, ...over });

/** Four repos, one feature name, main checkouts included as they are in a real scan. */
const scan = () => [
  { name: 'accept-blue', path: '/r/accept-blue', worktrees: [wt('accept-blue', '/r/accept-blue', { isMain: true }), wt('feat-x', '/r/accept-blue/.worktrees/feat-x')] },
  { name: 'merchant-v3', path: '/r/merchant-v3', worktrees: [wt('merchant-v3', '/r/merchant-v3', { isMain: true }), wt('feat-x', '/r/merchant-v3/.worktrees/feat-x')] },
  { name: 'ab-libraries', path: '/r/ab-libraries', worktrees: [wt('ab-libraries', '/r/ab-libraries', { isMain: true }), wt('feat-x', '/r/ab-libraries/.worktrees/feat-x')] },
  { name: 'other', path: '/r/other', worktrees: [wt('other', '/r/other', { isMain: true }), wt('unrelated', '/r/other/.worktrees/unrelated')] },
];
...
test('finds the feature worktrees the session has no record of', () => {
  ...
  assert.deepEqual(out.map((o) => o.repo), ['merchant-v3', 'ab-libraries']);
